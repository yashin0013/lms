...
class Admin extends CI_Controller
{
     ...
     public function demo_class()
    {
        ...
        $page_data['page_name'] = 'demo_class';
        $page_data['page_title'] = get_phrase('demo_class');
        $this->load->view('backend/index.php', $page_data);
    }
     public function add_meet()
    {
        ...
        $page_data['page_name'] = 'add_meet';
        $page_data['page_title'] = get_phrase('add_meet');
        $this->load->view('backend/index.php', $page_data);
    }
    public function insert_meet()
    {
       ...
       $data = array(
        ...
        $this->session->set_flashdata('flash_message', get_phrase('Meeing_inserted'));
         redirect(site_url('admin/demo_class'), 'refresh');
    }
     public function zoom($id="", $param1="")
    {
        
                 $this->db->set('zoom_id', $param1);
                $this->db->where('id', $id);
                $this->db->update('demo_class');
                redirect(site_url('admin/demo_class'), 'refresh');
    }
     public function manage_meet()
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        $page_data['page_name'] = 'manage_meet';
        $page_data['page_title'] = get_phrase('manage_meet');
        $this->load->view('backend/index.php', $page_data);
    }
      public function edit_meet()
    {
        ...
        $page_data['page_name'] = 'edit_meet';
        $page_data['page_title'] = get_phrase('edit_meet');
        $this->load->view('backend/index.php', $page_data);
    }
    
    public function update_meet($id)
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $this->session->set_flashdata('flash_message', get_phrase('Zoom_id_updated'));
        redirect(site_url('admin/manage_meet'), 'refresh');
    }
      ...
      public function delete_meet($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('zoom');
        $this->session->set_flashdata('flash_message', get_phrase('Zoom_id_Deleted'));
        redirect(site_url('admin/manage_meet'), 'refresh');
    }
     public function support()
    {
     ...
     public function status($param1 = "",$id="")
    {
        if ($param1 == '1') {
     ...
     public function teacher($id="", $param1="")
    {
        
                 $this->db->set('assigened_teacher', $param1);
                $this->db->where('id', $id);
                $this->db->update('demo_class');
                redirect(site_url('admin/demo_class'), 'refresh');
         ...
         public function supp_status($param1 = "",$id="")
    {
        if ($param1 == '1') {
            $this->db->set('status', 'approved');
                $this->db->where('id', $id);
        ...
        redirect(site_url('admin/support'), 'refresh');
    }
    //  public function add_course2()
    // {
    //      $id = $this->input->post('course');
    //     $d = $this->db->get_where('course', array('id'=> $id))->result_array();
    //     foreach ($d as $key){
    ...
    //     'price' => $key['price'],
    //     'discount_flag' => $key['discount_flag'],
    //     'discounted_price' => $key['discounted_price'],
    //     'level' => $key['level'],
    //     'user_id' => $key['user_id'],
    ...
    //     'is_top_course' => $key['is_top_course'],
    //     'is_admin' => $key['is_admin'],
    //     'status' => $key['status'],
    //     'course_overview_provider' => $key['course_overview_provider'],
    //     'meta_keywords' => $key['meta_keywords'],
    //     'meta_description' => $key['meta_description'],
    //     'is_free_course' => $key['is_free_course'],
    //     'multi_instructor' => $key['multi_instructor'],
    //     'creator' => $key['creator'],
    //     );
    ...
    //             redirect(site_url('admin/addon_course_category'), 'refresh');
                
        
    // }
      public function add_course2()
    {
        //  $id = $this->input->post('tag');
        // $d = $this->db->get_where('course', array('id'=> $id))->result_array();
    //   $n=$this->input->post('category_id');
    
        // $category_id['one'] = html_escape($this->input->post('category'));
        //     $category_id['two'] = html_escape($this->input->post('category_id'));
        //     $category_id['three'] = html_escape($this->input->post('category_id'));
    ...
    //  public function live_register_user()
    // {
    //     if ($this->session->userdata('admin_login') != true) {
    //         redirect(site_url('login'), 'refresh');
    //     }
    //     $page_data['page_name'] = 'live_register_user';
    //     $page_data['page_title'] = get_phrase('live_register_user');
    //     $this->load->view('backend/index.php', $page_data);
    // }
    ...
    public function categories($param1 = "", $param2 = "")
    {
        ...
        $page_data['categories'] = $this->crud_model->get_categories1($param2);
    ...
    public function category_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add_category") {
            ...
            $page_data['categories'] = $this->crud_model->get_categories1()->result_array();
     ...
     public function addon_course_category($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
          ...
          check_permission('addon');
            $page_data['page_name'] = 'addon_course_category';
            $page_data['page_title'] = get_phrase('addon_course_category');
             $this->load->view('backend/index',$page_data);
    }
 public function add_webinar($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            ...
            $page_data['page_title'] = get_phrase('add_webinar');
             $this->load->view('backend/index',$page_data);
    }
    
    public function manage_webinar($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
             
            $page_data['page_name'] = 'manage_webinar';
            $page_data['page_title'] = get_phrase('manage_webinar');
             $this->load->view('backend/index',$page_data);
    }
    
      public function webinar_info($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
             
            $page_data['page_name'] = 'webinar_info';
            $page_data['page_title'] = get_phrase('webinar_info');
             $this->load->view('backend/index',$page_data);
    }
    public function guest_list($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
             
            $page_data['page_name'] = 'guest_list';
            $page_data['page_title'] = get_phrase('Enrolled_students');
             $this->load->view('backend/index',$page_data);
    }
    ...
    public function user_form($param1 = "", $param2 = "")
    {
        ...
        elseif ($param1 == 'change_pass') {
                $page_data['page_name'] = 'change_pass';
            $page_data['user_id'] = $param2;
            $page_data['page_title'] = get_phrase('change_pass');
            $this->load->view('backend/index', $page_data);
        }
    }
     ...
     public function add_city()
    {
        ...
        if ($param1 == 'city') {
            ...
            redirect(site_url('admin/manage_institute'), 'refresh');
        }
        $page_data['page_name'] = 'add_city';
        $page_data['page_title'] = get_phrase('add_city');
        $this->load->view('backend/index', $page_data);
    }
    
     public function city_add($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
    ...
    public function add_institute()
    {
        ...
        $page_data['page_name'] = 'add_institute';
        $page_data['page_title'] = get_phrase('add_institute');
        $this->load->view('backend/index', $page_data);
    }
     public function institute_add($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
    ...
    public function manage_institute()
    {
        ...
        //     redirect(site_url('admin/manage_institute'), 'refresh');
        // }
        $page_data['page_name'] = 'manage_institute';
        $page_data['page_title'] = get_phrase('manage_institute');
        $this->load->view('backend/index', $page_data);
    }
//  public function manage_live()
//     {
//         if ($this->session->userdata('admin_login') != true) {
//             redirect(site_url('login'), 'refresh');
//         }
...
//         $page_data['page_name'] = 'manage_live';
//         $page_data['page_title'] = get_phrase('manage_live');
//         $this->load->view('backend/index', $page_data);
//     }
    ...
    public function enrol_multiple_students($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'students') {
            $this->crud_model->enrol_multiple_students_manually();
            redirect(site_url('admin/enrol_history'), 'refresh');
        }
        $page_data['page_name'] = 'enrol_multiple_students';
        $page_data['page_title'] = get_phrase('enrol_multiple_students');
        $this->load->view('backend/index', $page_data);
    }

public function enrol_multiple_instructors($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'instructors') {
            $this->crud_model->enrol_multiple_instructors_manually();
            redirect(site_url('admin/enrol_history1'), 'refresh');
        }
        $page_data['page_name'] = 'enrol_multiple_instructors';
        $page_data['page_title'] = get_phrase('enrol_multiple_instructors');
        $this->load->view('backend/index', $page_data);
    }

    public function course_list($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['page_title'] = get_phrase('course_list');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function enrol_multiple_courses($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'courses') {
            $this->crud_model->enrol_multiple_courses_manually();
            redirect(site_url('admin/enrol_history'), 'refresh');
        }
        $page_data['page_name'] = 'enrol_multiple_courses';
        $page_data['page_title'] = get_phrase('enrol_multiple_courses');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function purchase_report($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['page_name'] = 'purchase_report';
        $page_data['purchase_history'] = $this->crud_model->purchase_history1();
    ...
    public function course_actions($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            ...
            if (addon_status('live-class')) {
             ...
             if ($param1 == 'live_register_user') {
            $page_data['page_name'] = 'live_register';
            $page_data['page_title'] = get_phrase('live_register');
            $this->load->view('backend/admin/navigation', $page_data);
        } 
    ...
    public function course_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_course') {
            ...
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories1();
        ...
        elseif ($param1 == 'live_course_add') {
            $page_data['page_name'] = 'live_class';
            $page_data['page_title'] = get_phrase('live_course');
            $this->load->view('backend/index', $page_data);
        } 
    }
    ...
    {
       $data['user'] = html_escape($this->input->post('user_id'));
       if($data['user'] != 'all')
       {
        $data['user_id'] = $data['user']; 
        $data['msg']  = html_escape($this->input->post('msg'));
        $this->email_model->send_mail_market($data['user_id'], $data['msg']);
        $this->session->set_flashdata('flash_message', get_phrase('message_has_been_sent_successfully'));
        redirect(site_url('admin/email'), 'refresh');
        }
    ...
    //$res=  $data = $inputs_array =[];
    //     $to_name = $this->user_model->get_all_user()->result_array();
    //     foreach($to_name as $row){
    //         $data['uid'] = $row['id'];
    //         $inputs_array = array('user_id' => $data['uid']
    //     }
                //     print_r($input_array);die;
    // }
        //$data['user_id'] = $to_name['email'];
        //$data['user_id'] = implode(",", $to_name['email']);
        // $userid = array($to_name['email']);
        //foreach($userid as $uid){
       // $user_id[$value->email] = $value->email;
        //$res[$uid['email']]= $to_name['email'];
          //  }   
        //data array for view 
        //$data['user_id'] = $res;
        ...
        //  foreach($to_name as $row)
        //     {
        //     $data['user_id'] = $row->value[1];
        //     }
        //  //$data = array( 'user_id'=> $ids );
          
    
    
        //$data['user_id']   = isset($_GET['user_id']) ? $_GET['user_id'] : "all";
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'change_password1') {
            $this->user_model->change_password1($param2);
            redirect(site_url('admin/users'), 'refresh');
        }
