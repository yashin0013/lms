...
class Login extends CI_Controller {
    ...
    public function validate_login($from = "") {
        ...
        if ($query->num_rows() > 0) {
            ...
            // $this->session->set_flashdata('flash_message', get_phrase('welcome'.' '.$row->first_name));
      ...
      public function validate_login2($course = "",$id = "") {
        if($this->crud_model->check_recaptcha() == false && get_frontend_settings('recaptcha_status') == true){
            $this->session->set_flashdata('error_message',get_phrase('recaptcha_verification_failed'));
            redirect(site_url('home/login'), 'refresh');
        }
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($row->role_id == 1) {
                ...
                $this->session->set_userdata('user_login', '1');
                redirect(site_url('home/course/'.$course.'/'.$id), 'refresh');
            }
        }else {
            $this->session->set_flashdata('error_message',get_phrase('invalid_login_credentials'));
            redirect(site_url('home/login2/'.$course.'/'.$id), 'refresh');
        }
    }
   ...
   public function register() {
        ...
        if(empty($data['first_name']) || empty($data['last_name']) ||  empty($data['mobile']) || empty($data['email']) || empty($data['password'])){
        ...
        if($validity === 'unverified_user' || $validity == true) {
            ...
            if (get_settings('student_email_verification') == 'enable') {
                ...
                if($course){
                          ...
                          redirect(site_url('home/verification_code/'.$course.'/'.$courseId), 'refresh');
                    }else{
                          $this->session->set_userdata('register_email', $this->input->post('email'));
     ...
     public function register2($course="",$courseId="") {
        ...
        if($validity === 'unverified_user' || $validity == true) {
            ...
            if (get_settings('student_email_verification') == 'enable') {
                ...
                if($course){
                 ...
                 if($course){
                    $this->session->set_flashdata('flash_message', get_phrase('your_registration_has_been_successfully_done'));
                      redirect(site_url('home/login2/'.$course.'/'.$courseId), 'refresh');
                    }else{
                          $this->session->set_flashdata('flash_message', get_phrase('your_registration_has_been_successfully_done'));
                         redirect(site_url('home/login'), 'refresh');
                    }
