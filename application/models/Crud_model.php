...
class Crud_model extends CI_Model
{
    ...
    public function get_categories($param1 = "")
    {
        ...
        return $this->db->get('category', 5);
    }
     public function get_categories1($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('id', $param1);
              return $this->db->get('category');
    ...
    public function get_details_by_value($category_id=38,$sub_category_id=58,$price=0,$level="beginner",$language="english")
    {
//$this->db->select('*');
$this->db->from('category');
$this->db->join('course', ' ( category.id = course.category_id)');
$this->db->where('course.category_id',$category_id);
$this->db->where('course.sub_category_id',$sub_category_id);
$this->db->where('course.price', $price);
$this->db->where('course.level',$level);
$this->db->where('course.language',$language);
$query = $this->db->get();
    ...
    public function get_top_courses()
    {
        if (addon_status('scorm_course')) {
            ...
            return $this->db->get_where('course', array('is_top_course' => 1, 'status' => 'active', 'course_type' => 'nonpremium'));
        }
    }
    ...
    {     //$courses = 0;
        if (addon_status('scorm_course')) {
            if ($status != "") {
                ...
                //$courses = $this->db->count_all('course');
                //$courses++;
            } else {
            ...
            if ($status != "") {
                $courses = $this->db->get_where('course', array('status' => $status, 'course_type' => 'nonpremium'));
                
            } else {
                $courses['draft'] = $this->db->get_where('course', array('status' => 'draft', 'course_type' => 'nonpremium'));
                $courses['pending'] = $this->db->get_where('course', array('status' => 'pending', 'course_type' => 'nonpremium'));
                $courses['active'] = $this->db->get_where('course', array('status' => 'active', 'course_type' => 'nonpremium'));
    ...
    {
        if (addon_status('scorm_course')) {
            $this->db->where('course_type', 'nonpremium');
        }
