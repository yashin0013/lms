...
class Api_model extends CI_Model
{
	...
	//Book Demo Class 
	public function democlass($demodata)
	{
		$insert_data = array(
			'user_id' => $demodata['user_id'],
			'time' =>  $demodata['time'],
		
			'course_id' =>  $demodata['course_id'],
		);
		if ($this->db->insert('demo_class', $insert_data)) {
			$response['status'] = 'success';
			return $response;
		} else {
			$response['status'] = 'failed';
	...
	//Get Book Demo Class 
	public function getdemoclass($user_id)
	{
        return $this->db->get_where('demo_class',array('user_id'=>$user_id))->result_array();
  
	}
	
		//Get Book Demo Class 
	public function getsupportdata($user_id)
	{
        return $this->db->get_where('support',array('user_id'=>$user_id))->result_array();
  
	}
	
	//delete demo class
	public function deleteDemoClass($data)
	{
		$delete = array
		(
		...
		if ($this->db->delete('demo_class', $delete)) {
			$response['status'] = 'success';
			return $response;
		} else {
			$response['status'] = 'failed';
		...
		public function updateDemoClass($data)
	{
		...
		if ($this->db->update('demo_class', $update)) {
			$response['status'] = 'success';
			return $response;
		} else {
			$response['status'] = 'failed';
...
// 	public function getdemoclass($course_id = "")
// 	{
// 		if ($course_id != "") {
...
// 		$this->db->where('is_top_course', 1);
// 		$this->db->where('status', 'active');
// 		$demo_cses = $this->db->get('course')->result_array();
// 		// This block of codes return the required data of courses
// 		$result = array();
// 		$result = $this->course_data($top_courses);
// 		return $result;
// 	}
	...
	//Supprt
	public function support($demodata)
	{
		$insert_data = array(
			'user_id' => $demodata['user_id'],
// 			'user_email' => $demodata['user_email'],
			'msg' =>  $demodata['msg'],
		);
		if ($this->db->insert('support', $insert_data)) {
			$response['status'] = 'success';
			return $response;
		} else {
			$response['status'] = 'failed';
	  ...
	  public function liveClass($course_id)
  {
        return $this->db->get_where('live_class',array('course_id'=>$course_id))->result_array();
...
// public function login_post($login)
// {
//     $email=$login['email'];
//     $password=sha1($login['password']);
//     $value=array('email'=>$email,'password'=>$password);
//   return  $this->db->get_where('users',$value);
