...
class Home extends CI_Controller
{
    ...
    {
        // if (!$this->session->userdata('register_phone')) {
        //     redirect(site_url('home/sign_up'), 'refresh');
        // }
        $page_data['page_name'] = "verification_phone";
        $page_data['page_title'] = site_phrase('verification_phone');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function shopping_cart()
    {
        if ($this->session->userdata('user_login') != 1) {
           redirect(site_url('home/login2'), 'refresh');
        } else {
        ...
        if ($selected_category_id == "all" && $selected_price == "all" && $selected_level == 'all' && $selected_language == 'all' && $selected_rating == 'all') {
            ...
            // $courses = $this->crud_model->filter_course($selected_category_id, $selected_price, $selected_level, $selected_language, $selected_rating);
                    ...
                    if ($selected_category_id != "all") {
                      ...
                      $category_details= $this->db->get_where('category', array('id' => $selected_category_id))->result_array();
                    //   echo var_dump($category_details);
                    // $this->db->where('category_id', $selected_category_id);
                       // if ($category_details['parent'] > '0') {
                        if ($category_details['parent'] ==$d['sub_category_id']) {
                           $d= $this->db->where('sub_category_id', $selected_category_id);
                            //echo var_dump($d);
                        } else {
                            // $this->db->get_where('course', array('category_id' => $category_details['id'], 'status' => 'active'))->result_array();
                            $this->db->where('category_id', $selected_category_id);
                        }
                    }
                    ...
                    //     $course = $this->crud_model->get_rating_id($selected_rating);
                        // $course= $this->db->get_where('rating', array('rating' => $selected_rating))->result_array();
                        // $this->db->where('is', $course['ratable_id']);
                        // foreach($course as $key){
                            // $this->db->where('id', $key['ratable_id']);
                            // $c = $this->db->get_where('course', array('id' => $key['ratable_id']))->result_array();
                            // echo var_dump($c);
                        // }
                    ...
                    //     if (!addon_status('scorm_course')) {
                    //         $this->db->where('course_type', 'basic');
                    //     }
    ...
    public function courses2(){
        ...
        // $course =$this->db->get_where('course', array('id'=>  $course_id))->result_array();
        
            //  $page_data['courses'] = $course;
        // echo var_dump($page_data['courses']);
        
         $page_data['page_name']  = "courses3_page";
        $page_data['page_title'] = site_phrase('courses');
        $page_data['layout']     = $layout;
    ...
    public function set_layout_to_session()
    {
     ...
     public function web()
    {
        // $page_data['page_name'] = "web";
        // $page_data['page_title'] = site_phrase('web');
        // $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
        //  $id=5;
        // $live = $this->db->get('webinar')->result_array();
       $this->load->view('frontend/' . get_frontend_settings('theme') . '/web');
    ...
    public function course($slug = "", $course_id = "")
    {
      ...
      public function form($slug = "",$course_id)
        {
            if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home/login2/'.$slug.'/'.$course_id), 'refresh');
            }
            $page_data['course_id'] = $course_id;
            $page_data['page_name'] = "course_form";
            $page_data['page_title'] = site_phrase('course');
            $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
        }
         public function form_submit($slug = "", $course_id = "")
        {
             if ($this->session->userdata('user_login') != true) {
            ...
            // echo var_dump($data);
            $this->db->insert('demo_class', $data);
        //   redirect(site_url('home/course/'.$slug .'/' .$course_id), 'refresh');
        $this->session->set_flashdata('flash_message', site_phrase('Congratulations on successfully raising ticket for live demo class'));
             redirect(site_url('home/my_booked_courses'), 'refresh');
        }
        
         public function update_booked_courses($course_id = "")
        {
             if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home/login'), 'refresh');
            }
            ...
            $this->db->update('demo_class', $data);
            $this->session->set_flashdata('flash_message', site_phrase('Course_updated'));
           redirect(site_url('home/my_booked_courses'), 'refresh');
        }
        public function booked_courses_delete($course_id = "")
        {
             if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home/login'), 'refresh');
            }
            ...
            // echo var_dump($data);
            $this->db->where('id', $course_id);
            $this->db->delete('demo_class');
            $this->session->set_flashdata('flash_message', site_phrase('Course_Deleted'));
           redirect(site_url('home/my_booked_courses'), 'refresh');
        }
    ...
    public function my_courses()
    {
      ...
      public function my_booked_courses()
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }
        $page_data['page_name'] = "my_booked_courses";
        $page_data['page_title'] = site_phrase("my_booked_courses");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    public function booked_courses_edit($id="")
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }
        $page_data['page_name'] = "booked_courses_edit";
        $page_data['page_title'] = site_phrase("booked_courses_edit");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function mob_search(){
         $page_data['layout']     = $this->session->userdata('layout');
        $page_data['page_name'] = 'mob_search';
        $page_data['page_title'] = site_phrase('search');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function login2()
    {
        ...
        $page_data['page_name'] = 'login2';
        $page_data['page_title'] = site_phrase('login2');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
   }
    ...
    public function sign_up2()
    {
        ...
        $page_data['page_name'] = 'sign_up2';
        $page_data['page_title'] = site_phrase('sign_up2');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
